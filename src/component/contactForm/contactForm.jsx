...
class ContactForm extends Component {
  ...
  validate = () => {
    const schema = Joi.object({
      ...
      email: Joi.string()
        .email({ tlds: false })
        .messages({ "string.empty": "Field required" })
        .required()
        .label("Email"),
      mobNumber: Joi.string()
        ...
        .messages({
          "string.empty": "Field required",
          "string.pattern.base": "Only numbers",
  ...
  validateOnChange = (name, value) => {
    const schemas = {
      ...
      mobNumber: Joi.string()
        .custom((value, helper) => {
          if (value.length !== 10)
            return helper.message("Mobile number must be 10 characters");
      ...
      emailSubject: Joi.string()
        .custom((value, helper) => {
          if (!value.match(/^[a-z A-Z]+$/))
            return helper.message("Subject must be string of letters");
      ...
      <section data-aos="zoom-in" className="contactForm">
        ...
        <form onSubmit={this.handleSubmit}>
          ...
          <div>
            ...
            />
            {errors.mobNumber && (
              <p className="errorMessage">{errors.mobNumber}</p>
            )}
          </div>
          <div>
            <input
            ...
            />
            {errors.emailSubject && (
              <p className="errorMessage">{errors.emailSubject}</p>
            )}
          </div>
